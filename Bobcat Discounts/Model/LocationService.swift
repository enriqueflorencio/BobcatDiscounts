...
public class LocationService: NSObject, CLLocationManagerDelegate {
    ...
    // Sample Usage for above function ^
    // let businessAnnotation = locationService.createBusinessAnnotation("Little Oven", 27.02020202030304, 29.00202020202020)
    ...
    public func direct(businessLatitude: Double, businessLongitude: Double, callback: @escaping (CLLocationDistance) -> Void) {
        ...
        directions.calculate { [unowned self] (response, error) in
            ...
            ///Since this calculate method runs asynchronously, we're gonna need to use a callback (possibly)
            callback(distance)
            
            ///Could possibly use a delegate method to workaround this issue and update the locationpopup's miles label
...
//            }
    ...
    // Sample Usage for above function ^
    // let zoomedInRect = locationService.createRect(27.02020202030304, 29.00202020202020)
